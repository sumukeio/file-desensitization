...
1. **结构完备（Schema Preservation）**：表头、单元格格式、公式关系、日期时间、列类型完整保留；当用户主动勾选时，允许对列名与工作表名称做可选脱敏。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **表头行 (Headers)** | 顶部索引/规则判定 | `客户姓名` / `销售金额` | 默认保留；若用户勾选则按规则假名化 | 默认保留；若用户勾选则中间打星 | 默认保留最利于 AI 理解结构；仅在列名本身含敏感信息时建议处理 |
...
3. 🔒 **模式 3：绝密粉碎模式（极端保密专用）**
   ...
   - **具体怎么脱敏**：
     - 默认把表格里的文字内容粉碎成**随机乱码代号**（如 `ANON_8392`），完全看不出任何原始内容的字样；
     - 若您主动勾选“列名脱敏”或“工作表名称脱敏”，这些结构文字也会一起处理。
...
1. **批量上传与拖拽区 (Batch Dropzone)**：
   - 支持多文件同时拖入或点击选择；
   - 支持 `.xlsx`, `.xls`, `.csv`；
   - 支持表头位置：**自动识别（默认）** 或 **手动指定第 N 行**；
   - 支持两个默认关闭的结构开关：**是否对列名脱敏**、**是否对工作表名称脱敏**；
...
5. **最近更新提示 (What's New)**：
   - 顶栏提供固定「最近更新」入口，普通用户可随时查看近期功能变化；
   - 新版本首次进入自动提示一次，点“我知道了”后不再重复打扰；
   - 更新文案只写用户能直接感受到的变化，不堆技术术语。
...
- **布局划分**：
  - **顶部导航栏 (Header)**：产品 Logo、产品 Slogan（“面向大模型的智能文件脱敏工作站”）、「最近更新」入口、安全声明与隐私保证徽章。
  - **左侧/顶部规则控制台 (Config Panel)**：脱敏模式三档切换卡片（AI友好/标准掩码/深度匿名）、列名/工作表名称可选脱敏开关。
  - **主体核心操作区 (Main Area)**：
...
| [ ] 对列名（表头）脱敏    [ ] 对工作表名称（Sheet 名）脱敏                              |
|     默认不勾选；仅当这些结构文字本身含敏感信息时再打开                                     |
+-----------------------------------------------------------------------------------------+
|                                                                                         |
|      ┌ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ┐      |
|      |                        📂 点击或将 Excel 文件拖拽到此处                      |      |
...
### 阶段目标：
- **Phase 1（当前重点）**：聚焦 Excel（.xlsx/.xls/.csv），实现三档预设脱敏模式、批量上传、顺序队列排队处理、脱敏文件生成与单个/打包下载；现已补充可选列名 / 工作表名称脱敏与「最近更新」入口。
- **Phase 2**：加入“脱敏前后 Diff 实时可视化预览”、**智能识别表头所在行（自动识别 + 可手动指定）** 及“浏览器纯前端零上传运行支持”。
